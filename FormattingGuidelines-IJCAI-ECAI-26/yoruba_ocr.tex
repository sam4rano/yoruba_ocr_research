...
\begin{table}[t]
  ...
  \begin{tabular}{lrll}
    ...
    \bottomrule
  \end{tabular}
  \caption{Systems evaluated. ``Params'' denotes total model parameters. Tesseract model size is not publicly disclosed. $^\dagger$PP-OCRv4 metrics appear in data-size ablation (Table~\ref{tab:ablation}) from an archived evaluation; training checkpoints were not on disk at paper freeze and the row is omitted from Table~\ref{tab:main}. $^\ddagger$Surya and TrOCR are in the release pipeline (Phases~20--21); Table~\ref{tab:main} includes their rows when \texttt{metrics.csv} logs are present.}
  \label{tab:models}
\end{table}
...
\paragraph{Surya v2.}
Surya OCR v2 (0.9B parameters) is evaluated in \emph{recognition-only} mode: ground-truth line crops are passed directly to \texttt{RecognitionPredictor} with no layout or detection stage (\texttt{scripts/20\_baseline\_surya\_v2.py}; Phase~20). On NVIDIA GPUs the vLLM backend is used; on CPU or Apple Silicon, \texttt{llama.cpp} serves the shared VLM.

\paragraph{TrOCR.}
Microsoft TrOCR-large-printed (334M parameters) is fine-tuned on Yorùbá line crops with \texttt{scripts/21\_train\_trocr.py} and evaluated on the held-out test split (\texttt{scripts/22\_evaluate\_trocr.py}; Phase~21). Default training excludes the test partition (\texttt{--hold-out-test}).
...
Table~\ref{tab:main} lists all systems with on-disk inference logs on the export-stable test split (Table~\ref{tab:split}); Surya~v2 and TrOCR rows are populated after Phases~20--21 complete. Qwen~2.5-VL-3B is the release default (\S\ref{sec:models-qwen}); an archived 7B pilot characterises MLLM hallucination qualitatively. PP-OCRv4 ablation numbers use archived JSONL logs (see above). Document-level models such as Nougat remain out of scope. VL-1.5 LoRA used a single epoch on one NVIDIA T4 GPU without validation-driven early stopping.
