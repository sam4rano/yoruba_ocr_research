...
\end{equation}
Samples with $|d(y_i)|{=}0$ are excluded from $\mathrm{DER}_{\mathrm{corpus}}$; spurious predicted marks on such lines are summarised separately by a diacritic insertion rate.
\end{definition}
...
Initial transcription hypotheses were generated by PaddleOCR-VL-1.5 within the annotation platform; human annotators corrected every line. This introduces potential bias toward errors the pre-annotation model already makes. Inter-annotator agreement on diacritic edits was not quantified. The platform also applies automatic orthographic normalisation (e.g., uppercase headers), which may not match all downstream use cases.

\paragraph{Model coverage and training budget.}
We compare PP-OCRv4, Tesseract, PaddleOCR-VL-1.5, and Qwen~2.5-VL but do not include dedicated encoder--decoder OCR baselines (e.g., TrOCR, Nougat) that reviewers may expect. LoRA fine-tuning used a single epoch on one NVIDIA T4 GPU without a validation-driven early-stopping loop; additional epochs, larger adapter ranks, or multi-GPU training may change the supervised results.

\paragraph{Metric and evaluation scope.}
Corpus DER depends on the combining-mark set $\mathcal{U}$; sensitivity to alternative $\mathcal{U}$ definitions (e.g., treating precomposed \d{e}/\d{o}/\d{s} as atomic units) was not ablated. MLLM and zero-shot VLM scores are prompt-sensitive; we report one fixed template at temperature~0. DER correlates with diacritic fidelity but has not yet been validated against a large-scale human semantic-judgment study in this OCR setting.
