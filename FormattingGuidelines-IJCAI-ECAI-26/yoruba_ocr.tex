...
We introduce a curated corpus of 2{,}945 human-corrected line crops from the six-volume \emph{\Yoruba{} di W\'ur\`a} graded reader series, merged into a single corpus with export-stable train/validation/test partitions (2{,}367 / 252 / 326 lines). We benchmark classical OCR, vision-language models (VLMs), and a frozen Qwen~2.5-VL zero-shot run, and we propose \emph{Diacritic Error Rate} (DER) to isolate tone-mark errors from base-character corruption.
...
2{,}945 unique line crops. The benchmark release on Hugging Face preserves
export-stable partitions from the annotation platform:
\textbf{2{,}367 train / 252 validation / 326 test} lines (Table~1 metrics use
the 326-line test split). An optional consolidation pass with
\texttt{--resplit --seed 42} reassigns lines at 80/10/10 uniformly at random,
yielding 2{,}356 train, 294 validation, and 295 test lines for strict
line-level partitioning; we report results on the export-stable split unless
otherwise noted.
...
\begin{table}[t]
  ...
  \begin{tabular}{lrll}
    ...
    Qwen 2.5 VL & 7B & MLLM & Zero-shot (frozen run) \\
...
Qwen~2.5-VL-7B~\cite{qwen2025vl,bai2024qwen2vl} (7B parameters) was evaluated zero-shot once with a fixed instruction template and temperature~0; we do not re-run this baseline because the locked-in Table~\ref{tab:main} scores already characterise MLLM hallucination on diacritics (see \S\ref{sec:limitations}).
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrr}
    ...
    \bottomrule
  \end{tabular}
  \caption{Main results on the export-stable test split ($n{=}326$; corpus DER over $n_{\mathrm{der}}{=}319$ diacritic-bearing lines). Bold: best; underline: second-best among non-fine-tuned systems (Tesseract~(yor) DER). Error rates $>$100\% reflect hallucinated content exceeding reference length. PP-OCRv4 CRNN fine-tuning is reported separately in Table~\ref{tab:ablation} (archived eval; checkpoint not retained at paper freeze).}
  \label{tab:main}
\end{table}
...
Table~\ref{tab:main} reports scores on the export-stable test partition ($n{=}326$; 2{,}367 / 252 / 326 train/validation/test). An optional \texttt{--resplit --seed 42} pass yields a strict 80/10/10 line-level partition (2{,}356 / 294 / 295); applying it requires re-running all baselines. We report the export-stable split throughout.
