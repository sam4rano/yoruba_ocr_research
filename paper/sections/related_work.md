...
Recent multimodal LLMs report strong document parsing and text spotting on English-centric evaluation suites. OCR-oriented VLMs combine vision encoders with autoregressive language models, enabling them to leverage linguistic priors when reading text from images. Their behaviour on low-resource orthographies with contrastive diacritics is under-studied. A key question for Yorùbá OCR is whether contextual reasoning improves faithful transcription or introduces plausible but unfaithful text.
