...
| `paddleocrvl16_sft` | `scripts/train_paddleocrvl16_sft.py` and `scripts/eval_paddleocrvl16.py` via phases 14, 16, 17 | Supervised fine-tuned VLM | Fine-tunes PaddleOCR-VL-1.6 on the train split using assistant-only SFT loss. This is not zero-shot; it tests whether supervised adaptation improves faithful Yorùbá line transcription. |

Optional classical comparison:

| Model key | Script | Role |
| --- | --- | --- |
| `paddleocr_recognition_finetuned` | `scripts/train_paddleocr_recognition.py` + `scripts/05_evaluate.py` | Optional supervised PaddleOCR recognition fine-tune. Use this when comparing VLM SFT against a classical recognizer trained on the same labels. |
