...
Automation (`scripts/diagnose_experiment.py identity`) confirms labels load and metrics are self-consistent when prediction equals ground truth. **Content** quality still needs human eyes.

1. Run `python scripts/diagnose_experiment.py data --split test --sample 20 --seed 42`.
