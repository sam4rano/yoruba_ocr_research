...
| --- | --- | --- |
| `paddleocr_en_pretrained` | `scripts/06_baseline_pretrained.py` via `scripts/shell/phase_05_eval_paddleocr_recognition.sh` | Classical OCR control. Evaluates English-pretrained PP-OCR on Yorùbá while loading its matching English CTC head, so the row is not a random-head phantom. |
...
- `scripts/18_der_universe_ablation.py` recomputes DER under alternative
...
- `scripts/19_bootstrap_metric_cis.py` computes line-level bootstrap confidence
  intervals and aligned pairwise comparisons.
- `scripts/17_stratified_error_analysis.py` breaks errors down by linguistic
  features such as diacritic density and book/source groups.
- `scripts/12_diagnose_hypotheses.py` checks setup, checkpoint, and evaluation
...
`scripts/11_compile_results.py` excludes rows marked `phantom=true` or stale.
